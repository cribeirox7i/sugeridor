...
export default function RunScrapeButton() {
  ...
  async function run() {
    ...
    try {
      ...
      if (res.ok) {
        setState({ kind: "ok", msg: t("dispatchedOk") });
        // dá um tempo pro job 'running' aparecer e recarrega a lista
        setTimeout(() => router.refresh(), 4000);
      } else {
        setState({ kind: "error", msg: data.error ?? t("dispatchFail") });
      }
    } catch {
      setState({ kind: "error", msg: t("networkError") });
    }
  }
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {state.kind === "loading" ? t("running") : t("runButton")}
      </button>
      {state.msg && (
        <p
          className={
